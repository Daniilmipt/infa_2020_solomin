...
def sun(color_col,xcol,ycol,rcol):
    '''
    рисует солнце
    xcol-координата по ох центра солнца
    ycol-координата по оу центра солнца
    rcol-радиус солнца
    color_col-цвет солнца
    '''
    circle(screen, color_col, (xcol, ycol), rcol)
    

def poloci(color,a,b,c,d):
    '''
    Функция закрашивает полосу, на вход получает размеры этой полосы и её цвет
    '''
...
def bird(a, b, size): # project1
    ...
    На вход подаются координаты центра птицы и её рамзер
    ...
    вконце птица по координатам массива закрашивается через polygon
    '''
    q=[]
    w=[]
    ...
    polygon(screen, color_ptig, q) 
    polygon(screen, color_ptig, w)


def armia_bird(a,b,size):
    '''
    на вход подается координата первой птицы (а,b) и её рамзер
    по этим данным рисуются другие птицы через функцию bird
    '''
    bird(a, b, size)
...
'''
Далее идут функции рисования каждой из гор
вначале рисуются все её угловые части через polygon, потом для кривых заполняется массив координат через for и выводится через polygon
x0-смещение горы вдоль ох
y0-смещение горы вдоль оу
для каждой из кривых частей заполняется массив координат, по точкам которого уже рисуется фигура с помощью polygon
'''


def gora1(x0,y0):    
    '''
    рисуется первая гора
    x0-смещение горы вдоль ох, y0-смещение горы вдоль оу
    '''
    
     # рисует все угловые части горы
    ...
    # рисует ПРАВУЮ кривую часть первой горы
    mountain_1 = []
    for i in range(480+x0, 550+x0, 1):
        mountain_1.append((i, (-25 / 2000) * (i - 480-x0) ** 2 + 161+y0)) 
    for i in range(550+x0, 600+x0, 1):
        mountain_1.append((i, 0.029 * (i - 580-x0) ** 2 + 74+y0)) 
    mountain_1.append((600+x0, 189+y0)) 
    mountain_1.append((480+x0, 200+y0))
    polygon(screen, color_gor1, mountain_1)

     # рисует ЛЕВУЮ кривую часть первой горы
    mountain_2 = [] 
    for i in range(0+x0, 165+x0, 1):
        mountain_2.append((i, (-109 / 21904) * (i - 17-x0) ** 2 + 217+y0)) 
    mountain_2.append((165+x0, 230+y0))
    mountain_2.append((0+x0, 245+y0))
    polygon(screen, color_gor1, mountain_2)


def gora2(x01,y01):
    '''
    рисуется вторая гора
    x01-смещение горы вдоль ох, y01-смещение горы вдоль оу
    '''
     # рисует угловые части горы
...
def gora3(x03,y03):
    '''
    рисуется третья гора
    x03-смещение горы вдоль ох, y03-смещение горы вдоль оу
    '''
    ...
    polygon(screen, color_gor3, mountain_6)
    ...
    polygon(screen, color_gor3, mountain_7)

'''
Эталоннные значения для ввода:
800
530
110
125
...
poloci(color_osn,0,0,x,y) # основной фон
poloci(color_1,0,y1,x,y2) # полоса в середине
sun( color_col, xcol, ycol, rcol)
gora1(x0,y0)
gora2(x01,y01)
